...
def base_data(db):
    ...
    from finance.models import Account, BankAccount
    # Currency
    ...
    for code, name, typ in account_defs:
        Account.objects.get_or_create(code=code, defaults={"name":name, "type":typ, "is_active":True})
    # Bank
    bank, _ = BankAccount.objects.get_or_create(name="Main Bank", defaults={"account_code":"1000", "currency":aed, "active":True})
    # VAT (AE 5%)
