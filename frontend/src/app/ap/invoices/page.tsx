...
import { apInvoicesAPI, invoiceApprovalsAPI } from '../../../services/api';
...
export default function APInvoicesPage() {
  ...
  const handleSubmitForApproval = async (invoiceId: number, invoiceNumber: string) => {
    ...
    try {
      const response = await apInvoicesAPI.submitForApproval(invoiceId, {
    ...
    <div className="max-w-7xl mx-auto">
      <div className="mb-8 flex justify-between items-center">
        <div>
          ...
          <p className="mt-2 text-gray-600">Manage supplier invoices</p>
        </div>
        <Link href="/ap/invoices/new" className="btn-primary flex items-center gap-2">
          ...
          <tbody className="divide-y divide-gray-200">
            {invoices.map((invoice) => (
              <tr key={invoice.id} className="hover:bg-gray-50">
                ...
                <td className="table-td">{invoice.supplier_name || `Supplier #${invoice.supplier}`}</td>
                ...
                {/* Column 1: Posting Status (Posted/Unposted/Cancelled) */}
                <td className="table-td">
                  {invoice.is_cancelled ? (
                    ...
                    <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
                      invoice.is_posted ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                    }`}>
                ...
                {/* Column 2: Payment Status (show for all non-cancelled invoices) */}
                <td className="table-td">
                  {!invoice.is_cancelled ? (
                    <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
                      invoice.payment_status === 'PAID' ? 'bg-blue-100 text-blue-800'
                      : invoice.payment_status === 'PARTIALLY_PAID' ? 'bg-purple-100 text-purple-800'
                ...
                {/* Column 3: Approval Status (only when approval_status exists) */}
                <td className="table-td">
                  {invoice.approval_status ? (
                    <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
                      invoice.approval_status === 'APPROVED' ? 'bg-emerald-100 text-emerald-800'
