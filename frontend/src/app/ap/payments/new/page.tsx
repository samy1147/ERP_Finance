  ...
  const fetchOutstandingInvoices = async (supplierId: number) => {
    ...
    try {
      ...
      const allocations: InvoiceAllocation[] = invoiceData.map((inv: any) => ({
        ...
        outstanding: inv.outstanding || '0',  // Backend returns 'outstanding', not 'outstanding_amount'
        amount: '',
        selected: false,
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-5xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">New AP Payment</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="bg-white shadow-md rounded-lg p-6">
            <h2 className="text-xl font-semibold text-gray-900 mb-6">
              Invoice Allocations
              {!formData.supplier && <span className="text-sm text-gray-500 font-normal ml-2">(Select a supplier first)</span>}
            </h2>

            {/* Multi-Currency Info Box */}
            {formData.supplier && invoices.length > 0 && (() => {
              const paymentCurrency = currencies.find(c => c.id === parseInt(formData.currency));
              const paymentCurrencyCode = paymentCurrency?.code || '';
              const baseCurrency = currencies.find(c => c.is_base)?.code || 'AED';
            ...
            {loadingInvoices ? (
              ...
              <div className="text-center py-8 text-gray-500">
                {formData.supplier ? 'No outstanding invoices found for this supplier' : 'Select a supplier to view outstanding invoices'}
              </div>
            ) : (
