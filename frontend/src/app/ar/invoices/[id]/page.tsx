...
export default function ViewARInvoicePage() {
  ...
  const fetchInvoice = async () => {
    try {
      setLoading(true);
      const response = await arInvoicesAPI.get(parseInt(invoiceId));
  ...
  const handlePost = async () => {
    ...
    if (window.confirm('Are you sure you want to post this invoice? This action cannot be undone.')) {
      try {
        await arInvoicesAPI.post(invoice.id);
  ...
  const handleSubmitForApproval = async () => {
    ...
    if (window.confirm(`Submit invoice ${invoice.invoice_number} for approval?`)) {
      try {
        await arInvoicesAPI.submitForApproval(invoice.id, {
  ...
  const handleDelete = async () => {
    ...
    if (window.confirm('Are you sure you want to delete this invoice?')) {
      try {
        await arInvoicesAPI.delete(invoice.id);
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mb-8 flex justify-between items-start">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">AR Invoice #{invoice.invoice_number}</h1>
      ...
      <div className="card mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <p className="mt-1 text-base text-gray-900">{invoice.customer_details?.name || invoice.customer_name || 'N/A'}</p>
          </div>
