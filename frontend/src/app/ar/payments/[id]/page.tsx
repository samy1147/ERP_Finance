...
export default function ViewARPaymentPage() {
  ...
  const handlePost = async () => {
    ...
    if (window.confirm('Are you sure you want to post this payment?')) {
      try {
        await arPaymentsAPI.post(payment.id);
  ...
  const handleDelete = async () => {
    ...
    if (window.confirm('Are you sure you want to delete this payment?')) {
      try {
        await arPaymentsAPI.delete(payment.id);
      ...
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">AR Payment Details</p>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          {!isPosted && (
            <>
              <button
                onClick={() => router.push(`/ar/payments/${payment.id}/edit`)}
      ...
      <div className="bg-white shadow-md rounded-lg p-6 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <p className="mt-1 text-base text-gray-900">{payment.customer_name || 'N/A'}</p>
          </div>
