...
export default function ARPaymentsPage() {
  ...
  const fetchPayments = async () => {
    try {
      const response = await arPaymentsAPI.list();
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-8 flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">AR Payments</h1>
          <p className="mt-2 text-gray-600">Receive payments from customers</p>
        </div>
        <Link href="/ar/payments/new" className="btn-primary flex items-center gap-2">
        ...
        <table className="table-base">
          <thead className="table-header" style={{ backgroundColor: '#f9fafb' }}>
            <tr>
              ...
              <th className="table-th" scope="col" style={{ padding: '12px 24px', fontSize: '12px', fontWeight: 600, color: '#374151', textTransform: 'uppercase' }}>Customer</th>
            ...
            {payments.map((payment) => {
              ...
              return (
                <tr key={payment.id} className="hover:bg-gray-50">
                  ...
                  <td className="table-td">{payment.customer_name || `Customer #${payment.customer}`}</td>
