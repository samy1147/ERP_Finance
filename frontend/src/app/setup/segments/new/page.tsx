...
export default function NewSegmentPage() {
  ...
  const loadInitialData = async () => {
    try {
      ...
      setSegmentTypes(typesData.filter(t => t.is_active));
      setSegments(segmentsData);
      
      // Set default segment type from URL if present
      const typeFromUrl = searchParams?.get('type');
      if (typeFromUrl) {
        setFormData(prev => ({ ...prev, segment_type: typeFromUrl }));
      }
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await segmentService.createSegment(submitData);
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-1">Add a new segment value</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-6 space-y-6">
        ...
        <div>
          ...
          <input
            ...
            onChange={handleChange}
            required
            maxLength={selectedType?.length || 50}
            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            placeholder="Enter unique code"
          />
          {selectedType?.length && (
            <p className="mt-1 text-sm text-gray-500">
              Fixed length: {selectedType.length} characters (pad with zeros if needed)
            </p>
          )}
        </div>
